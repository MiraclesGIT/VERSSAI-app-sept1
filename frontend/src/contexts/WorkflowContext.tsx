...
// 3-Layer RAG Architecture Types
interface RAGLayer {
  id: string;
  name: string;
  description: string;
...
interface WorkflowContextType {
  ...
  ragLayers: RAGLayer[];
  activeRagLayer: string;
  setActiveRagLayer: (layerId: string) => void;
  updateRagLayer: (layerId: string, updates: Partial<RAGLayer>) => void;
  
  // Workflow Execution
  executions: WorkflowExecution[];
  startWorkflow: (workflowType: string, parameters: any) => Promise<string>;
  getExecution: (executionId: string) => WorkflowExecution | null;
  subscribeToExecution: (executionId: string, callback: (execution: WorkflowExecution) => void) => void;
  
  // N8N Integration
  n8nWorkflows: N8NWorkflow[];
  triggerN8NWorkflow: (workflowId: string, data: any) => Promise<string>;
  getN8NStatus: () => Promise<boolean>;
  
  // MCP Agent Integration
  mcpConnected: boolean;
  sendMCPMessage: (message: any) => Promise<any>;
}
...
export const WorkflowProvider: React.FC<WorkflowProviderProps> = ({ children }) => {
  const [ragLayers, setRagLayers] = useState<RAGLayer[]>([]);
  const [activeRagLayer, setActiveRagLayerState] = useState<string>('roof');
  ...
  useEffect(() => {
    ...
    const initialRagLayers: RAGLayer[] = [
      {
        id: 'roof',
    ...
    const initialN8nWorkflows: N8NWorkflow[] = [
      {
  ...
  const setActiveRagLayer = (layerId: string) => {
  ...
  const updateRagLayer = (layerId: string, updates: Partial<RAGLayer>) => {
    setRagLayers(prev => prev.map(layer => 
      layer.id === layerId ? { ...layer, ...updates } : layer
    ));
  };

  const startWorkflow = async (workflowType: string, parameters: any): Promise<string> => {
  ...
  const getExecution = (executionId: string): WorkflowExecution | null => {
    return executions.find(exec => exec.id === executionId) || null;
  };

  const subscribeToExecution = (executionId: string, callback: (execution: WorkflowExecution) => void) => {
  ...
  const triggerN8NWorkflow = async (workflowId: string, data: any): Promise<string> => {
  ...
  const updateN8nWorkflow = (workflowId: string, updates: Partial<N8NWorkflow>) => {
    setN8nWorkflows(prev => prev.map(workflow => 
      workflow.id === workflowId ? { ...workflow, ...updates } : workflow
    ));
  };

  const getN8NStatus = async (): Promise<boolean> => {
    const n8nUrl = process.env.REACT_APP_N8N_URL || 'http://localhost:5678';
